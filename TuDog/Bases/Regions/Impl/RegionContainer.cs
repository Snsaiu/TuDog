...
public class RegionContainer : RegionContainerBase
{
    ...
    public override void TryAdd(string regionName, ContentControl region)
    {
        _regions[regionName] = region;
    }

    public override void Remove(string regionName)
    {
    ...
    {
        if (_regions.TryGetValue(regionName, out var region)) return region;
        throw new NullReferenceException($"Region {regionName} not found");
    }

    public override bool Exists(string regionName)
    {
