...
public class ViewLocatorDictionaryGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateSource(IEnumerable<VmModel> models)
    {
        ...
        sb.AppendLine("{");
        sb.AppendLine("[TuDog.IocAttribute.Register<TuDog.Interfaces.IViewLocatorService>(TuDog.IocAttribute.ServiceLifetime.Singleton)]");
        sb.AppendLine("public sealed class ViewLocatorService:TuDog.Interfaces.IViewLocatorService {");

        sb.AppendLine("public System.Collections.Generic.IDictionary<System.Type, System.Func<Avalonia.Controls.Control>> InitControlDictionaryControls(){");
    ...
    private static IEnumerable<VmModel> FindClassesByAttribute(INamespaceSymbol symbol)
    {
        ...
        foreach (var type in GetNamespaceTypes(symbol))
            if (type is { TypeKind: TypeKind.Class } and var item)
            {
                ...
                if (item.GetAttributes().FirstOrDefault(x => x.AttributeClass?.OriginalDefinition.ToDisplayString() == "TuDog.IocAttribute.NoViewAttribute") is not null)
                    continue;
