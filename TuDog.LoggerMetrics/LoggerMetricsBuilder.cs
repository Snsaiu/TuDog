...
public static class LoggerMetricsBuilder
{
    ...
    public static void AddLoggerBuilder(this IContainer services, string? lokiUrl, IEnumerable<LokiLabel>? lokiLabels, string logDirectory)
    {
        ...
        var logger = new LoggerConfiguration().MinimumLevel.Debug().Enrich.FromLogContext().WriteTo.Console(theme: AnsiConsoleTheme.Code).WriteTo.File(logPath, rollingInterval: RollingInterval.Day);
